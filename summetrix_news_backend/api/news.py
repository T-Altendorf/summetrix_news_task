...
def get_news():
    try:
        ...
        if response.status_code == 200:
            ...
            return (
                jsonify(
                    {"error": "Failed to fetch news data", "data": response.json()}
                ),
...
def add_to_favorites():
    try:
        ...
        return (
            jsonify(
                {"message": "News added to favorites successfully", "data": result}
            ),
            201,
        )
...
def resource_not_found(e):
    return (
        jsonify(
            error=str(e.description), message="The requested resource was not found"
        ),
        404,
    )
